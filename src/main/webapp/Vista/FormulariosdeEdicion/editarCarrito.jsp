<%-- 
    ...
    Created on : 25 oct 2024, 17:45:00
    Author     : Joaquin
--%>
...
<% 
    ...
    if (carrito == null) {
%>
    <p>El carrito no se encontró. Por favor, inténtelo de nuevo.</p>
<% 
    return; 
    }
%>
    ...
    <body>
        ...
        <div class="form-container">
            ...
            <form action="<%= request.getContextPath() %>/MainControler" method="post">
                <input type="hidden" name="action" value="actualizarCarrito">
                <input type="hidden" name="idCarrito" value="<%= carrito.getIdCarrito() %>">

                <div class="grupo-input">
                    <label for="userId">ID Usuario:</label>
                    <input type="text" id="userId" name="userId" value="<%= carrito.getUserId() %>" required>
                </div>

                <div class="grupo-input">
                    <label for="florId">ID Flor:</label>
                    <input type="text" id="florId" name="florId" value="<%= carrito.getFlorId() %>" required>
                </div>

                <div class="grupo-input">
                    <label for="cantidad">Cantidad:</label>
                    <input type="number" id="cantidad" name="cantidad" value="<%= carrito.getCantidad() %>" required>
                </div>

                <div class="grupo-input">
                    <label for="precio">Precio:</label>
                    <input type="text" id="precio" name="precio" value="<%= carrito.getPrecio() %>" required>
                </div>
